...
def create_comprehensive_dataframe(df_apartments_with_bfe, all_sales_data):
    ...
    # Fix datetime parsing with error handling for invalid dates
    print("-> Converting dates...")
    
    # First, let's inspect problematic dates
    print(f"-> Sample of raw dates: {df_all_sales['dato'].head()}")
    
    # Convert with error handling - coerce invalid dates to NaT
    df_all_sales['dato'] = pd.to_datetime(df_all_sales['dato'], errors='coerce', utc=True)
    
    # Check for and remove invalid dates
    invalid_dates = df_all_sales['dato'].isna()
    if invalid_dates.sum() > 0:
        print(f"-> Found {invalid_dates.sum()} invalid dates, removing them...")
        df_all_sales = df_all_sales[~invalid_dates].copy()
    
    # Also filter out dates that are too old (before 1900) or too new (after 2030)
    date_filter = (df_all_sales['dato'].dt.year >= 1900) & (df_all_sales['dato'].dt.year <= 2030)
    if (~date_filter).sum() > 0:
        print(f"-> Found {(~date_filter).sum()} dates outside reasonable range (1900-2030), removing them...")
        df_all_sales = df_all_sales[date_filter].copy()
    
    print(f"-> Remaining sales after date cleaning: {len(df_all_sales)}")
...
def main():
    ...
    json_output_path = './all__nonapartment_sales_history.json'
    ...
    output_csv = 'comprehensive_non_apartment_sales.csv'
